...
/* Adapted from original_flashfloppy/src/image/mfm.c. */
static const uint16_t s_floppy_mfm_table[256] =
{
...
{
    return s_floppy_mfm_table[value];
}
...
void floppy_mfm_encode_bytes(const uint8_t *in, uint16_t *out, size_t len)
{
    ...
    while (index < len)
    {
        out[index] = floppy_mfm_encode_byte(in[index]);
        index++;
    }
}
